(() => {
  ...
  let onboardingNicknameError = "";
  let onboardingSecurityAccepted = false;
  let onboardingCurseForgeKey = "";
  let onboardingInstallConfirmations = 0;

  const onboardingStorageKey = "cloudy-onboarding-step";
  const onboardingSecurityKey = "cloudy-security-accepted";
  const onboardingCurseForgeStorageKey = "cloudy-curseforge-key";
  try {
    onboardingSecurityAccepted = localStorage.getItem(onboardingSecurityKey) === "true";
    onboardingCurseForgeKey = localStorage.getItem(onboardingCurseForgeStorageKey) || "";
  } catch (_) {}

  const readOnboardingStep = () => {
    try { return Math.min(10, Math.max(1, Number(localStorage.getItem(onboardingStorageKey) || 1))); } catch (_) { return 1; }
  };
  const saveOnboardingStep = (step) => {
    onboardingStep = Math.min(10, Math.max(1, step));
  ...
  const renderOnboarding = (currentState) => {
    ...
    const stepLabels = uiLanguage() === "ru" ? ["Язык", "Стиль", "Java", "Диск", "Безопасность", "Иконки", "CurseForge", "Связи", "Accounts", "Установка"] : ["Language", "Style", "Java", "Storage", "Security", "Icons", "CurseForge", "Links", "Accounts", "Install"];
    ...
    if (step === 1) {
      ...
      content = `<div class="onboarding-step-copy security-onboarding"><p class="eyebrow">05 / ${ru ? "Безопасность" : "Security"}</p><h2>${ru ? "Безопасность Cloudy Launcher" : "Cloudy Launcher security"}</h2><p class="lead">${ru ? "Перед завершением настройки подтвердите условия использования и состав поставки лаунчера." : "Before finishing setup, confirm the usage terms and what is included with the launcher."}</p><div class="security-card"><div class="security-icon security-shield" aria-hidden="true"></div><div><strong>${ru ? "Лицензионное соглашение" : "License agreement"}</strong><p>${ru ? "Cloudy Launcher распространяется по GPL-3.0. Исходный код, условия лицензии и сведения об авторах доступны в разделе «О лаунчере»." : "Cloudy Launcher is distributed under GPL-3.0. The source code, license terms and attribution are available in About Cloudy Launcher."}</p></div></div><div class="security-list"><div><span class="security-app-icon security-yandex-icon" aria-hidden="true"></span><p><strong>${ru ? "Без Яндекс Браузера" : "No Yandex Browser"}</strong><small>${ru ? "Лаунчер не устанавливает и не запускает Яндекс Браузер." : "The launcher does not install or launch Yandex Browser."}</small></p></div><div><span class="security-app-icon security-360-icon" aria-hidden="true"></span><p><strong>${ru ? "Без 360 Total Security" : "No 360 Total Security"}</strong><small>${ru ? "360 Total Security и любые рекламные установщики не входят в поставку." : "360 Total Security and advertising installers are not bundled."}</small></p></div><div><span class="security-app-icon security-check-icon" aria-hidden="true"></span><p><strong>${ru ? "Прозрачная поставка" : "Transparent distribution"}</strong><small>${ru ? "Cloudy не добавляет скрытые программы. Проверяйте подпись релиза и исходный код перед установкой." : "Cloudy adds no hidden programs. Check release signatures and source before installation."}</small></p></div></div><label class="security-consent" data-onboard-action="security"><input id="onboard-security" type="checkbox" ${onboardingSecurityAccepted ? "checked" : ""}><span>${ru ? "Я прочитал(а) условия лицензии и подтверждаю, что понимаю состав установки Cloudy Launcher." : "I have read the license terms and understand what is included in the Cloudy Launcher installation."}</span></label></div>`;
    } else if (step === 6) {
      const iconCards = [
        ["Cloudy Vanilla", "cloudy", "Vanilla"],
        ["Cloudy Fabric", "fabric", "Fabric"],
        ["Cloudy Quilt", "quilt", "Quilt"],
        ["Cloudy Forge", "forge", "Forge"],
      ];
      content = `<div class="onboarding-step-copy"><p class="eyebrow">06 / ${ru ? "Иконки сборок" : "Instance icons"}</p><h2>${ru ? "Предпросмотр иконок для сборок" : "Preview your instance icons"}</h2><p class="lead">${ru ? "Так будут выглядеть сборки в библиотеке Cloudy. Иконки можно будет заменить позже." : "This is how instances will appear in the Cloudy library. You can replace icons later."}</p><div class="instance-icon-preview-grid">${iconCards.map(([name, key, type]) => `<div class="instance-icon-preview"><span class="preview-instance-icon preview-${key}">${key === "cloudy" ? "C" : type.slice(0,1)}</span><strong>${name}</strong><small>${type}</small></div>`).join("")}</div><div class="callout">${ru ? "Иконки не устанавливают сторонние программы и используются только для оформления сборок." : "These icons are visual assets only and do not install third-party software."}</div></div>`;
    } else if (step === 7) {
      content = `<div class="onboarding-step-copy"><p class="eyebrow">07 / CurseForge API</p><h2>${ru ? "Подключите CurseForge API" : "Connect the CurseForge API"}</h2><p class="lead">${ru ? "Для поиска и установки проектов CurseForge нужен ваш официальный API-ключ. Cloudy не выдаёт и не подменяет ключи." : "Your official API key is required for CurseForge project search and installation. Cloudy does not generate or replace keys."}</p><div class="api-key-card"><span class="provider-mark provider-curseforge">CF</span><div><strong>${ru ? "Официальный API-ключ" : "Official API key"}</strong><small>${ru ? "Создайте ключ в" : "Create your key at"} <a href="https://console.curseforge.com" target="_blank" rel="noopener noreferrer">console.curseforge.com</a></small></div></div><label class="form-label" for="onboard-curseforge-key">${ru ? "CurseForge API-ключ" : "CurseForge API key"}</label><input class="form-input api-key-input" id="onboard-curseforge-key" type="password" autocomplete="off" value="${escapeHtml(onboardingCurseForgeKey)}" placeholder="${ru ? "Вставьте ключ сюда" : "Paste your key here"}"><div class="callout">${ru ? "Ключ сохраняется в настройках Cloudy и используется только для запросов CurseForge." : "The key is stored in Cloudy settings and used only for CurseForge requests."}</div></div>`;
    } else if (step === 8) {
      content = `<div class="onboarding-step-copy"><p class="eyebrow">08 / ${ru ? "Наши связи" : "Our links"}</p><h2>${ru ? "Открытый проект Cloudy" : "Cloudy is an open project"}</h2><p class="lead">${ru ? "Исходный код, история изменений и сообщения об ошибках доступны в официальном репозитории." : "Source code, changes and issue reports are available in the official repository."}</p><a class="project-link-card" href="https://github.com/manusportalgpt-beep/Cloudy-Launcher" target="_blank" rel="noopener noreferrer"><span class="github-mark">GH</span><span><strong>GitHub.com / Cloudy-Launcher</strong><small>${ru ? "Открыть репозиторий" : "Open repository"}</small></span><span class="external-arrow">↗</span></a><div class="callout">${ru ? "Cloudy Launcher сохраняет GPL-3.0 и upstream attribution." : "Cloudy Launcher preserves GPL-3.0 and upstream attribution."}</div></div>`;
    } else if (step === 9) {
      const accountReady = currentState.hasAccount;
      content = `<div class="onboarding-step-copy"><p class="eyebrow">09 / ${ru ? "Аккаунты" : "Accounts"}</p><h2>${ru ? "Выберите способ входа" : "Choose how to continue"}</h2><p class="lead">${ru ? "Microsoft сохраняет лицензионный вход через настоящий OAuth/device-code flow. Никнейм создаёт локальный offline-профиль без лицензии." : "Microsoft uses the real OAuth/device-code flow. A nickname creates a local offline profile without a license."}</p><div class="auth-choice-grid"><button class="auth-choice auth-choice-microsoft ${onboardingAuth === "microsoft" || accountReady ? "is-selected" : ""}" data-onboard-action="microsoft"><span class="auth-icon auth-microsoft"><i></i><i></i><i></i><i></i></span><span><strong>${ru ? "Microsoft аккаунт" : "Microsoft account"}</strong><small>${accountReady ? (ru ? "Аккаунт подключён" : "Account connected") : (ru ? "OAuth и лицензия" : "OAuth and license")}</small></span></button><button class="auth-choice auth-choice-nickname ${onboardingAuth === "nickname" ? "is-selected" : ""}" data-onboard-action="nickname"><span class="auth-icon auth-shirt"></span><span><strong>${ru ? "По никнейму" : "Nickname"}</strong><small>${ru ? "Только локальная игра" : "Local/offline play"}</small></span></button></div>${onboardingAuth === "nickname" ? `<div class="nickname-form"><label for="onboard-nickname">${ru ? "Никнейм (3–16 символов)" : "Nickname (3–16 characters)"}</label><div class="inline-form"><input class="form-input" id="onboard-nickname" maxlength="16" pattern="[A-Za-z0-9_]{3,16}" value="${escapeHtml(onboardingNickname)}" placeholder="CloudyPlayer"><button class="action-button primary" data-onboard-action="create-nickname">${ru ? "Создать профиль" : "Create profile"}</button></div>${onboardingNicknameError ? `<p class="form-error" role="alert">${escapeHtml(onboardingNicknameError)}</p>` : ""}<small>${ru ? "Локальный профиль сохраняется в native accounts.json и доступен после перезапуска." : "The local profile is saved by the native account manager and remains after restart."}</small></div>` : ""}<div class="callout">${accountReady ? (ru ? "Профиль готов. Лицо скина будет взято native account manager из UUID, если профиль предоставляет его." : "Your profile is ready. The native account manager supplies the UUID skin face when available.") : (ru ? "Выберите Microsoft или никнейм. Можно продолжить позже, не подменяя реальные учётные данные." : "Choose Microsoft or nickname. You can continue later without fabricating credentials.")}</div></div>`;
    } else if (step === 10) {
      const confirmCopy = [
        ["Вы действительно хотите скачать лаунчер?", "Do you really want to download the launcher?"],
        ["Вы УВЕРЕНЫ?", "Are you SURE?"],
        ["Вы ТОЧНО уверены?", "Are you ABSOLUTELY sure?"],
        ["ВЫ ТОЧНО ДЕЙСТВИТЕЛЬНО УЛЬТРА СУПЕР ГОТОВЫ К УСТАНОВКЕ?", "ARE YOU REALLY ULTRA SUPER READY TO INSTALL?"],
      ];
      const current = confirmCopy[Math.min(3, onboardingInstallConfirmations)];
      content = `<div class="onboarding-step-copy install-confirmation"><p class="eyebrow">10 / ${ru ? "Финальная проверка" : "Final check"}</p><h2>${ru ? current[0] : current[1]}</h2><p class="lead">${ru ? "Это шуточная защитная последовательность перед завершением настройки. Никаких скрытых загрузок не выполняется." : "This is a playful confirmation sequence before setup completes. No hidden downloads are performed."}</p><div class="confirmation-meter"><span style="width:${onboardingInstallConfirmations * 25}%"></span></div><p class="confirmation-count">${onboardingInstallConfirmations} / 4</p><button class="action-button primary confirmation-button" data-onboard-action="install-confirm">${ru ? "Да" : "Yes"}</button><div class="callout">${ru ? "После четвёртого подтверждения мастер можно будет завершить." : "After the fourth confirmation, setup can be completed."}</div></div>`;
    } else {
      const accountReady = currentState.hasAccount;
      content = `<div class="onboarding-step-copy"><p class="eyebrow">09 / ${ru ? "Авторизация" : "Account"}</p><h2>${ru ? "Выберите способ входа" : "Choose how to continue"}</h2><p class="lead">${ru ? "Microsoft сохраняет лицензионный вход через настоящий OAuth/device-code flow. Никнейм создаёт локальный offline-профиль без лицензии." : "Microsoft uses the real OAuth/device-code flow. A nickname creates a local offline profile without a license."}</p><div class="auth-choice-grid"><button class="auth-choice auth-choice-microsoft ${onboardingAuth === "microsoft" || accountReady ? "is-selected" : ""}" data-onboard-action="microsoft"><span class="auth-icon auth-microsoft"><i></i><i></i><i></i><i></i></span><span><strong>${ru ? "Microsoft аккаунт" : "Microsoft account"}</strong><small>${accountReady ? (ru ? "Аккаунт подключён" : "Account connected") : (ru ? "OAuth и лицензия" : "OAuth and license")}</small></span></button><button class="auth-choice auth-choice-nickname ${onboardingAuth === "nickname" ? "is-selected" : ""}" data-onboard-action="nickname"><span class="auth-icon auth-shirt"></span><span><strong>${ru ? "По никнейму" : "Nickname"}</strong><small>${ru ? "Только локальная игра" : "Local/offline play"}</small></span></button></div>${onboardingAuth === "nickname" ? `<div class="nickname-form"><label for="onboard-nickname">${ru ? "Никнейм (3–16 символов)" : "Nickname (3–16 characters)"}</label><div class="inline-form"><input class="form-input" id="onboard-nickname" maxlength="16" pattern="[A-Za-z0-9_]{3,16}" value="${escapeHtml(onboardingNickname)}" placeholder="CloudyPlayer"><button class="action-button primary" data-onboard-action="create-nickname">${ru ? "Создать профиль" : "Create profile"}</button></div>${onboardingNicknameError ? `<p class="form-error" role="alert">${escapeHtml(onboardingNicknameError)}</p>` : ""}<small>${ru ? "Локальный профиль сохраняется в native accounts.json и доступен после перезапуска." : "The local profile is saved by the native account manager and remains after restart."}</small></div>` : ""}<div class="callout">${accountReady ? (ru ? "Профиль готов. Лицо скина будет взято native account manager из UUID, если профиль предоставляет его." : "Your profile is ready. The native account manager supplies the UUID skin face when available.") : (ru ? "Выберите Microsoft или никнейм. Можно продолжить позже, не подменяя реальные учётные данные." : "Choose Microsoft or nickname. You can continue later without fabricating credentials.")}</div></div>`;
    }
    const stepper = stepLabels.map((label, index) => `<button class="onboarding-step ${index + 1 === step ? "is-current" : index + 1 < step ? "is-done" : ""}" data-onboard-action="jump" data-step="${index + 1}" ${index + 1 > step ? "disabled" : ""}><b>${String(index + 1).padStart(2, "0")}</b><small>${label}</small></button>`).join("");
    const onboardingNextDisabled = (step === 5 && !onboardingSecurityAccepted) || (step === 10 && onboardingInstallConfirmations < 4) ? "disabled" : "";
    workspace.innerHTML = `<section class="page onboarding-page"><div class="onboarding-shell"><div class="onboarding-header"><div><p class="eyebrow">Cloudy setup</p><h1>${ru ? "Настройте рабочее пространство" : "Set up your workspace"}</h1><p class="lead">${ru ? "Десять коротких шагов. Настройки записываются native лаунчером и не исчезают после выхода." : "Ten short steps. Choices are written by the native launcher and survive restart."}</p></div><span class="onboarding-badge">${String(step).padStart(2, "0")} / 10</span></div><div class="onboarding-stepper">${stepper}</div><div class="onboarding-content">${content}</div><div class="onboarding-footer"><button class="action-button quiet" data-onboard-action="back" ${step === 1 ? "disabled" : ""}>${ru ? "Назад" : "Back"}</button><span class="onboarding-footer-note">${ru ? "Можно изменить позже в настройках" : "You can change these later in Settings"}</span><button class="action-button primary" data-onboard-action="next" ${onboardingNextDisabled}>${step === 10 ? (currentState.hasAccount ? (ru ? "Открыть Cloudy" : "Open Cloudy") : (ru ? "Продолжить позже" : "Continue later")) : (ru ? "Далее" : "Continue")}</button></div></div></section>`;
  };
  ...
  document.addEventListener("input", (event) => {
    if (event.target?.id === "onboard-curseforge-key") {
      onboardingCurseForgeKey = event.target.value;
      try { localStorage.setItem(onboardingCurseForgeStorageKey, onboardingCurseForgeKey); } catch (_) {}
      callNative("setCurseForgeApiKey", onboardingCurseForgeKey);
    }
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    if (onboardingButton) {
      ...
      if (action === "jump") {
        const targetStep = Number(onboardingButton.dataset.step || 1);
        if (targetStep <= onboardingStep) {
          saveOnboardingStep(targetStep);
          render();
        }
      } else if (action === "install-confirm") {
        onboardingInstallConfirmations = Math.min(4, onboardingInstallConfirmations + 1);
        render();
      } else if (action === "security") {
        const checkbox = document.getElementById("onboard-security");
        onboardingSecurityAccepted = Boolean(checkbox?.checked);
        try { localStorage.setItem(onboardingSecurityKey, String(onboardingSecurityAccepted)); } catch (_) {}
        render();
        ...
        render();
      } else if (action === "curseforge-key") {
        onboardingCurseForgeKey = value;
        try { localStorage.setItem(onboardingCurseForgeStorageKey, onboardingCurseForgeKey); } catch (_) {}
        callNative("setCurseForgeApiKey", onboardingCurseForgeKey);
        ...
        onboardingNicknameError = "";
        render();
      } else if (action === "create-nickname") {
        const input = document.getElementById("onboard-nickname");
        onboardingNickname = input?.value.trim() || "";
        if (!/^[A-Za-z0-9_]{3,16}$/.test(onboardingNickname)) {
          onboardingNicknameError = copy("Use 3–16 letters, numbers or underscores.", "Используйте 3–16 латинских букв, цифр или символов подчёркивания.");
          render();
        } else {
          onboardingNicknameError = "";
          callNative("addOfflineAccount", onboardingNickname);
          onboardingAuth = "";
          render();
        }
      } else if (action === "back") {
