(() => {
  ...
  try { onboardingSecurityAccepted = localStorage.getItem(onboardingSecurityKey) === "true"; } catch (_) {}

  const readOnboardingStep = () => {
    try { return Math.min(6, Math.max(1, Number(localStorage.getItem(onboardingStorageKey) || 1))); } catch (_) { return 1; }
  };
  ...
  const renderOnboarding = (currentState) => {
    ...
    const stepLabels = uiLanguage() === "ru" ? ["Язык", "Стиль", "Java", "Диск", "Безопасность", "Аккаунт"] : ["Language", "Style", "Java", "Storage", "Security", "Account"];
    ...
    const onboardingNextDisabled = step === 5 && !onboardingSecurityAccepted ? "disabled" : "";
    workspace.innerHTML = `<section class="page onboarding-page"><div class="onboarding-shell"><div class="onboarding-header"><div><p class="eyebrow">Cloudy setup</p><h1>${ru ? "Настройте рабочее пространство" : "Set up your workspace"}</h1><p class="lead">${ru ? "Шесть коротких шагов. Настройки записываются native лаунчером и не исчезают после выхода." : "Six short steps. Choices are written by the native launcher and survive restart."}</p></div><span class="onboarding-badge">${String(step).padStart(2, "0")} / 06</span></div><div class="onboarding-stepper">${stepper}</div><div class="onboarding-content">${content}</div><div class="onboarding-footer"><button class="action-button quiet" data-onboard-action="back" ${step === 1 ? "disabled" : ""}>${ru ? "Назад" : "Back"}</button><span class="onboarding-footer-note">${ru ? "Можно изменить позже в настройках" : "You can change these later in Settings"}</span><button class="action-button primary" data-onboard-action="next" ${onboardingNextDisabled}>${step === 6 ? (currentState.hasAccount ? (ru ? "Открыть Cloudy" : "Open Cloudy") : (ru ? "Продолжить позже" : "Continue later")) : (ru ? "Далее" : "Continue")}</button></div></div></section>`;
  };
